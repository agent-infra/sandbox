...
> 这些示例刻意把宿主机侧绑定到 `127.0.0.1`，因为沙盒容器内部监听的是 `0.0.0.0`。如果部署在云主机或 Kubernetes 上，请保持 `8080` 为私网端口，并通过反向代理或 Ingress 对外发布。参见[云上部署指南](/zh/guide/start/cloud-deployment)。

> 如果端口 8080 被占用，映射到不同的本地回环端口：
