...
The path lives under `/cdp/`, which the deployment serves, not the daemon. See [CDP Access](/daemon/basic/cdp). Which SDK calls fail against the daemon is listed in [1.x SDK compatibility](/daemon/start/migration#1x-sdk-compatibility).
