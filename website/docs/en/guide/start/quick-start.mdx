...
```bash
docker run --security-opt seccomp=unconfined --rm -it -p 127.0.0.1:8080:8080 ghcr.io/agent-infra/sandbox:1.0.0.150

# or users in mainland China
docker run --security-opt seccomp=unconfined --rm -it -p 127.0.0.1:8080:8080 enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:1.0.0.150
```

> These examples intentionally bind the host side to `127.0.0.1` because the sandbox listens on `0.0.0.0` inside the container. If you are deploying on a cloud VM or Kubernetes, keep `8080` private and publish it through a reverse proxy or Ingress. See [Cloud Deployment](/guide/start/cloud-deployment).

> If port 8080 is occupied, map to a different loopback port:
